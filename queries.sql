...
-- Read as a 7-day trailing average of daily order revenue (total_amt_usd).
WITH daily_sales AS (
  SELECT DATE(occurred_at) AS order_day,
         SUM(total_amt_usd) AS day_sales
  FROM orders
  GROUP BY DATE(occurred_at)
)
SELECT order_day,
       day_sales,
       ROUND(AVG(day_sales) OVER (
